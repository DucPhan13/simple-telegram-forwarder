...
with open("./config/fwd_config.yml", "r") as ymlfile:
    cfg = yaml.load(ymlfile, Loader=yaml.SafeLoader)

api_id = cfg['telethon']['api_id']
api_hash = cfg['telethon']['api_hash']
destination_channel_id = cfg['telethon']['dst_id']
keywords = cfg['telethon']['keywords']
...
async def handler(event):
    if signals_on == True and any([obj.get('id') == event.chat_id and obj.get('data').get('enabled') == True for obj in channels['channels']]):
        await forward_messages_to_channel(destination_channel_id, event.text, keywords)
...
async def forward_messages_to_channel(destination_channel_id, message, keywords):
    ...
    if keywords:
        ...
        if message and any(keyword in message.lower() for keyword in keywords):
            print(f"Message contains a keyword: {message}")

            # Forward the message to the destination channel
            await client.send_message(destination_channel_id, message)

            print("Message forwarded: " + message)
    else:
        # Forward the message to the destination channel
        await client.send_message(destination_channel_id, message)
